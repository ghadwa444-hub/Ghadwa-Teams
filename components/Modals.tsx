  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authError) {
        ...
        logger.error("AUTH_MODAL", "❌ Login failed", {
          error: authError,
          code: (authError as any)?.code,
          message: errorMessage,
        });

        if ((authError as any)?.code === "invalid_credentials" || errorMessage.includes("Invalid login credentials")) {
          setError("بيانات الدخول غير صحيحة. تأكد من البريد الإلكتروني وكلمة المرور.");
        } else if (errorMessage.includes("Email not confirmed")) {
          setError("البريد الإلكتروني غير مفعّل. يرجى تفعيله أولاً.");
        } else {
      ...
      if (!user || !profile) {
        ...
        logger.error("AUTH_MODAL", "❌ Login failed - no user or profile", { user, profile });
        setLoading(false);
        return;
      }
      ...
      setError(`حدث خطأ في تسجيل الدخول: ${errorMessage}`);
      ...
      <div className="bg-white w-full max-w-md rounded-2xl overflow-hidden relative z-10 shadow-2xl animate-fade-in">
        ...
        <div className="p-8">
          <div className="text-center mb-8">
            <div className="w-12 h-12 bg-[#8B2525] rounded-full flex items-center justify-center text-white font-bold text-2xl mx-auto mb-3">
              غ
            </div>
      ...
      <div className="bg-white w-full max-w-sm rounded-2xl overflow-hidden relative z-10 p-6 text-center animate-fade-in shadow-2xl">
        ...
        <div className="space-y-3">
          <button
            ...
            className="w-full flex items-center justify-between p-4 rounded-xl border border-gray-200 hover:border-[#8B2525] hover:bg-red-50 transition group"
          >
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="bg-white w-full max-w-lg rounded-2xl overflow-hidden relative z-10 shadow-2xl animate-fade-in">
        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-full bg-white text-gray-500 hover:text-red-500 hover:bg-red-50 flex items-center justify-center transition"
          >
            <i className="fa-solid fa-xmark"></i>
          </button>
        </div>
        <form onSubmit={onSubmit} className="p-6 space-y-4">
          ...
          <div className="pt-4 flex gap-3">
            <button
              type="submit"
              className="flex-1 bg-[#8B2525] text-white py-3 rounded-xl font-bold hover:bg-[#6b1c1c] transition"
            >
              حفظ البيانات
            </button>
            <button
